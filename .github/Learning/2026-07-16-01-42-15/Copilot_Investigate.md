...
The current source tree has no `NetworkProtocolHttp.*`, `AsyncSocket_HttpServerApi.*`, `AsyncSocket_HttpClientApi.*`, or `TestInterProcess_AsyncSocket_MiniHttpApi.cpp` files and no `SocketHttpServerApi`, `SocketHttpClientApi`, or `SocketHttpRequestContext` declarations. The required public API and behavior therefore cannot compile or execute before implementation. The existing Debug x64 solution is the positive baseline and builds with zero warnings and zero errors.

Add the requested shared test file and make it the single cross-platform registration point. Its shared runner must use the native server/client type pair supplied by each platform guard and cover:

- portable Windows-shaped HTTP values, UTF-8 body helpers, shared URL query encoding/decoding, and common route constants;
- prefix validation and normalization, per-request longest-prefix dispatch on one persistent connection, duplicate rejection, root fallback, 404 behavior, shared listener survival after one prefix stops, final port release, deterministic concurrent startup handoff, and bounded occupied-port failure;
- immediate and deferred responses, write-completion delivery, cancellation, disconnect/stop races, hard callback draining, multi-chunk fixed-length output, framing rejection, and most-derived destruction discipline;
- sequential binary `POST`, `GET`, `HEAD`, ordinary `OPTIONS`, preflight, FIFO submission, callback-reentrant submission, exact host/identity fields, CORS/date/cache fields, connection reuse, and rejection of unsupported client options;
- parseable `400`, `404`, `405`, `408`, `413`, `414`, `415`, `417`, `431`, `500`, `501`, and `505` outcomes through native exchanges or deterministic private seams;
- both additional Windows interoperability directions on ports 38901 and 38902.

Success requires the focused new Debug x64 test file and the complete Debug x64 suite to pass with no CRT leak dump; Debug/Release and Win32/x64 builds to finish with zero warnings and errors; and a textual cross-platform audit of common-header purity, platform guards, registry synchronization, prefix parsing, callbacks, UTF-8 handling, and query handling.

# PROPOSALS

- No.1 Build the MiniHttp layer by extending the existing byte HTTP exchange

## No.1 Build the MiniHttp layer by extending the existing byte HTTP exchange

Keep the completed `HttpRequestConnection`, `HttpRequestServer`, and `HttpRequestClient` as the only HTTP/1.1 parser, serializer, connection, and callback-lifetime implementation. Add the minimized application policy as composition around those objects, and extend the lower layers only where the required behavior cannot be expressed by composition.

### COMMON HTTP DEPENDENCY

Add platform-neutral `Source/InterProcess/NetworkProtocolHttp.h/.cpp`. Preserve `windows_http::HttpRequest`, `windows_http::HttpResponse`, and `windows_http::HttpError` exactly in their current namespace while moving their declarations and UTF-8 body helpers out of the Windows header and implementation. Change only `HttpError::errorCode` to `vuint32_t`. Move the three `HttpServerUrl_*` constants unchanged into the common header.

Expose one common URL query encoder and decoder from this header. Preserve the existing Windows policy exactly: encode UTF-8, leave only ASCII alphanumeric characters unescaped, emit uppercase `%HH`, decode valid escapes, preserve malformed escapes literally, and translate `+` to space. Both `HttpClientApi` and `SocketHttpClientApi` retain their static helpers as thin wrappers around these common functions. Strict prefix/path decoding is a separate implementation helper because it must reject malformed escapes, invalid UTF-8, NUL, and encoded slash or backslash instead of applying form-query behavior.

The common header depends only on common declarations. `NetworkProtocol.Windows.h` includes it and retains Windows-only headers and declarations. The common socket client header depends on both this common value header and the raw HTTP client header, with colliding request and response names fully qualified.

### REQUEST-LAYER EXTENSIONS

Add only the following platform-neutral seams to the existing async-socket and byte HTTP layers:

- `AsyncSocketServerStartFailure` and `AsyncSocketServerStartException` distinguish address-in-use from another listener startup failure without parsing exception text. Native Windows and Linux setup failures use the typed exception; macOS waits for its asynchronous listener ready/failure result so `Start` has the same contract on every platform.
- `IAsyncSocketServerCallback::OnServerStopped` is a default no-op terminal listener notification. Native servers invoke it once, outside locks, for an unexpected terminal listener failure. `HttpRequestServer` relays it through a protected virtual hook and its existing callback-draining domain, suppressing derived notification during an explicit stop.
- `HttpRequestFailure` identifies `400`, `408`, `413`, `414`, `417`, `431`, `501`, and `505` terminal request failures. `IHttpRequestCallback::OnReadRequestFailure` permits one final server response and forces connection close after its write completion. Malformed or unsafe client responses remain transport errors.
- `IHttpRequestConnection::SendRequest` accepts a per-exchange response deadline with the existing 30-second behavior as the default and non-positive values meaning infinite. The client deadline is not refreshed by partial response input; the server's incomplete-message inactivity deadline retains refresh behavior.
- Store the active request method so a client ends a `HEAD` response after its fields and a server serializes representation fields without body octets. Treat `204` and `304` as bodyless while preserving legal representation `Content-Length` semantics. Reject unsupported transfer codings as `501` for server requests and as client transport failure.
- Inspect `Expect` after the complete header section and before waiting for a declared body, producing `417`. A fully parsed server request continues to disable the incomplete-message timer while the application owns a response context.

All new request, response, failure, completion, disconnect, and listener callbacks reserve state under locks and invoke user code only after releasing locks. Application-layer dispatch and completion callbacks catch exceptions so the lower connection always advances its exchange state.

### FINAL PUBLIC API

The application surface follows the authoritative design without a public listener injection constructor:

- `SocketHttpRequestContext` has an implementation-only constructor, out-of-line destructor, deleted copy/move operations, `GetRequest`, `GetRelativePath`, `GetQuery`, one-shot `Respond(Ptr<async_tcp_socket::HttpResponse>, Func<void(bool)> = {})`, and one-shot `Cancel`.
- `SocketHttpServerApi(const WString& urlPrefix, bool respondToOptions)` has deleted copy/move operations, virtual destructor, protected pure `OnHttpRequestReceived`, protected virtual `OnHttpServerStopping`, and public `Start`, idempotent hard-drain `Stop`, `IsStopped`, and `GetUrlPrefix`.
- `SocketHttpClientApi(Ptr<IAsyncSocketClient> client, const WString& authority)` has deleted copy/move operations, destructor hard stop, `WaitForServer`, `GetStatus`, FIFO `HttpQuery(const windows_http::HttpRequest&, Func<void(Variant<windows_http::HttpResponse, windows_http::HttpError>)>)`, `Stop`, and static query encode/decode wrappers.

Every most-derived server destructor must call `Stop` before destroying callback-visible state. The base destructor performs only a non-virtual safety stop.

### REGISTRY, DISPATCH, AND RESPONSE LIFETIME

Use a process-wide registry keyed by port, with one global `SpinLock` protecting the registry pointer/map and entry-reference publication only. Each reference-counted entry owns one specialized `HttpRequestServer`, its native `IAsyncSocketServer`, connection dispatchers, and normalized prefix registrations. Listener construction/start, waiting, retry, stop, drain, and all callbacks happen outside the global lock.

The first starter publishes an in-progress generation and creates the listener outside the lock. Followers retain that generation and wait on its manual-reset event. Address-in-use rechecks the registry and either joins a published winner or installs a fresh generation, with at most five actual bind attempts. A private function-pointer listener factory seam supports the shared native test harness and deterministic startup barriers without changing the public constructor.

Normalize constructor prefixes without binding. Require `http://`, a supported loopback authority, explicit valid port, and a strict decoded absolute path; remove all trailing slashes. `Start` is one-shot. Reject duplicate normalized prefixes, permit nesting, validate exactly one matching `Host`, and select the longest case-sensitive segment-boundary match for every completed request. The root prefix is the fallback. Valid unmatched requests receive `404`, globally unknown methods receive `501`, and `OPTIONS *` is handled from the entry's active policy.

A connection dispatcher retains each accepted `IHttpRequestConnection` callback relationship but performs prefix lookup again for every request. Each dispatched `SocketHttpRequestContext` retains the completed byte request, decoded relative path, raw query, API lifetime, and connection dispatcher. The owning API tracks its active contexts. `Respond` validates and flattens chunks to fixed length, rejects trailers/application transfer encoding/contradictory length, applies HEAD/204/304 body rules, and adds Date, no-store, and wildcard CORS defaults. Its completion runs exactly once only after the matching raw write completion. `Cancel`, disconnect, API stop, and shared-listener stop race through one context state transition and report failed delivery outside locks.

Stopping one API removes only its prefix, cancels and drains only its contexts, and leaves the shared listener running. The last API removes the entry while globally locked, then stops and drains the shared server outside the lock. An unexpected listener notification marks every attached API stopped, cancels contexts, invokes stopping callbacks outside locks, and removes the dead entry.

The socket client owns one `HttpRequestClient` and one FIFO queue. It converts supported Windows-shaped requests into raw byte requests, generates exactly one `Host` and `Accept-Encoding: identity`, rejects secure/credential/keep-alive options and conflicting Host, maps only `receiveTimeout` to the exchange deadline, flattens response chunks, and completes every accepted callback at most once. HTTP statuses remain successful responses; connection, framing, deadline, cancellation, unsupported coding, and local programming failures become `HttpError`.
