...
The adapter uses the same **/VlppInterProcess/Connect**, **/VlppInterProcess/Request/{token}** and **/VlppInterProcess/Response/{token}** routes as **windows_http::HttpServer** and **windows_http::HttpClient**. The protocol has no message-level delivery acknowledgement, deduplication, heartbeat or disconnect route, so it must not be treated as exactly-once delivery. If a server cannot deliver a pending Request response, it restores the message to the FIFO head and reports a nonfatal local error. A raw callback may leave it queued for a replacement poll, while an admitted **NetworkProtocolChannelServer** connection promotes the error and disconnects because channel delivery is no longer reliable.
...
**vl::inter_process::windows_http::HttpServer** creates a **vl::inter_process::windows_http::HttpServerConnection** for each connect request. Server-to-client messages are returned through a pending long-poll request when possible, or queued until the next request. If that pending response fails because the polling connection was lost, the message is retained and **OnLocalError** receives a recoverable error; an admitted channel promotes it and stops the logical connection. Client-to-server request bodies are dispatched as inbound strings. When the server stops, pending long-poll requests are cancelled and connection callbacks receive disconnection.
