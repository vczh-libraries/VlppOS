...
Windows Terminal is the required Windows host for this project's visual verification of bold, italic, underline and strikeline. The user manually confirmed all four effects in Windows Terminal on both Windows 10 and Windows 11 on 2026-09-05. Windows 11 is not required: the terminal host supplies the visual rendering. The earlier underline-only result belongs to the tested built-in console host (`conhost.exe`), not Windows 10 generally. Linux text styles were also manually confirmed. See the [verification results by terminal host](../Jobs/DebugTuiPlaygroundSOP.md#2026-09-05-manual-text-style-verification-by-terminal-host).
