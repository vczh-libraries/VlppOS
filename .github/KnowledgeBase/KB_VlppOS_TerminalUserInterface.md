...
Windows Terminal 1.11 or later provides all four effects with [configurable intense-text formatting](https://devblogs.microsoft.com/commandline/windows-terminal-preview-1-11-release/); prefer the current stable release. Launch the playground from a shell tab inside that app so Windows Terminal performs the visible rendering. The current Windows Terminal [OS requirement](https://github.com/microsoft/terminal#installing-and-running-windows-terminal) is Windows 10 version 2004 (build 19041) or newer, including Windows 11. This is the terminal application's requirement, separate from VlppOS's Windows 10 development baseline.

On 2026-09-05, the user manually verified visible bold, italic, underline and strikeline on Windows 11, following an earlier successful Linux check. The Windows 11 terminal host/version, font, profile settings and exact OS build were not supplied. This confirms the effects in the tested environment without making an OS-wide rendering guarantee. The earlier Windows 10 inbox-conhost result remains underline-only. See the [SOP verification record](../Jobs/DebugTuiPlaygroundSOP.md#2026-09-05-windows-11-manual-text-style-verification) for the distinction between manual visual results and automated attribute checks.
