...
The refined implementation passed ten focused inter-process processes and a complete-suite smoke test, but the next clean-build campaign aborted on protocol process six. A parent GDB run with a C++ throw catchpoint reproduced it and exposed the swallowed worker exception before the watchdog: Tom's task threw at the `Connected` assertion immediately after `eventTom.Wait()`. Tom's callback sends `Stop` and signals `eventTom`; Jerry does the same. The server can receive both messages, signal `eventServer`, call `server->Stop()`, and disconnect both clients before Tom or Jerry wakes and performs its own status assertion and local `Stop`. This is a legal cross-connection order that the test incorrectly assumes away. Add a second server-side gate so the server waits until both client tasks have completed their connected assertion and local connection `Stop` before stopping the listener. This preserves the status and shutdown checks instead of weakening them.
