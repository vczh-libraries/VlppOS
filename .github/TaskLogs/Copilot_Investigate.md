...
Create `Test/UnitTest/MiniHttpServer/{MiniHttpServer.vcxproj,MiniHttpServer.vcxproj.filters,Main.cpp}` and the seven files under `Test/MiniHttpServer/{Website,Assets}`. Add the project and four configuration pairs to `Test/UnitTest/UnitTest.sln`. Move the existing Unix UnitTest configuration to `Test/Linux/UnitTest`, create and generate `Test/Linux/MiniHttpServer`, and update `Project.md` plus the active Linux path in `README.md`. Create the ignored local `.vcxproj.user` only for execution. Do not modify shared product source or product unit tests.
