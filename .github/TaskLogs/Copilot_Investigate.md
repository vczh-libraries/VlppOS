...
- Linux replaces `ServerState`'s concrete owner with the callback pointer. `Start` installs it with `startCalled`/`starting`; the existing start scope clears it whenever startup does not reach a running state. Accept completion increments `activeCallbacks` while copying the pointer under `lockState`, then invokes outside the lock. `Stop` prevents later entries, waits down to the current thread's callback depth, and clears the pointer only when called outside that callback. A nested stop therefore returns without waiting for itself, and a later external stop observes zero callback depth, drains, and clears.
...
Change the shared interface, `NetworkProtocolServer` bridge, all three native server declarations and implementations, every shared async-socket test server-start path, the focused Windows callback-boundary tests, and `TODO_SocketHttp_AsyncSocket.md` as described above. No new source file, build metadata, generated file, client API, connection callback contract, framing behavior, or unrelated transport code will change.
